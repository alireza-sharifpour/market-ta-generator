...
from app.external.llm_client import escape_markdownv2, generate_combined_analysis
...
class LLMCache:
    ...
    def generate_data_hash(
        ...
        try:
            ...
            cache_data = {
                ...
                "sr_levels": {
                    "support": [
                        round(level, 4) for level in sr_levels.get("support", [])
                    ],
                    "resistance": [
                        round(level, 4) for level in sr_levels.get("resistance", [])
                    ],
                },
    ...
    def generate_cache_key(
        ...
        try:
            data_hash = self.generate_data_hash(
                df_with_indicators, sr_levels, timeframe
            )
    ...
    def replace_price_placeholders(
        self, analysis: Dict[str, str], current_price: Optional[float]
    ) -> Dict[str, str]:
        """
        ...
        try:
            ...
            for key, content in analysis.items():
                if isinstance(content, str):
                    ...
                    updated_content = updated_content.replace(
                        CACHE_PLACEHOLDERS["current_price"], escaped_price_str
                    )
            ...
            logger.debug(
                f"Replaced placeholders with current price: {current_price_str} (escaped: {escaped_price_str})"
            )
            return result
    ...
    async def get_or_generate(
        ...
        try:
            ...
            cache_key = self.generate_cache_key(
                pair, df_with_indicators, sr_levels, timeframe
            )
            ...
            if cached_analysis:
                ...
                analysis = self.replace_price_placeholders(
                    cached_analysis, current_price
                )
            ...
            logger.info(
                f"Cache MISS for {pair} ({timeframe}) - generating new analysis"
            )

            # Prepare LLM input WITHOUT current price for cache consistency
            structured_llm_input = prepare_llm_input_for_cache(
    ...
    async def _generate_without_cache(
        ...
        try:
            logger.warning(
                f"Generating analysis without cache for {pair} ({timeframe})"
            )
            ...
            if current_price is not None:
                current_price_data = {"ticker": {"latest": current_price}}
