...
def test_enhanced_sr_detection():
    ...
    for timeframe in timeframes:
        ...
        try:
            ...
            logger.info(f"Support levels found: {sr_levels['support']}")
            logger.info(f"Resistance levels found: {sr_levels['resistance']}")
            ...
            support_near_target = [
                level for level in sr_levels['support'] 
                if abs(level - target_level) <= tolerance
            ]
            
            if support_near_target:
                logger.info(f"✅ SUCCESS: Found support level(s) near {target_level}: {support_near_target}")
            else:
                logger.info(f"❌ Target level {target_level} not detected in timeframe {timeframe}")
                
                # Show closest levels for debugging
                if sr_levels['support']:
                    closest_support = min(sr_levels['support'], key=lambda x: abs(x - target_level))
                    logger.info(f"Closest support level: {closest_support} (distance: {abs(closest_support - target_level):.2f})")
    ...
    try:
        ...
        logger.info(f"Sensitive - Support levels: {sr_levels_sensitive['support']}")
        logger.info(f"Sensitive - Resistance levels: {sr_levels_sensitive['resistance']}")
        
        # Check again for the target level
        support_near_target = [
            level for level in sr_levels_sensitive['support'] 
            if abs(level - 2525) <= 5
        ]
        
        if support_near_target:
            logger.info(f"✅ SUCCESS with sensitive params: Found support level(s) near 2525: {support_near_target}")
        else:
            logger.info(f"❌ Target level 2525 still not detected with sensitive parameters")
